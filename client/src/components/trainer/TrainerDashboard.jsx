...
const TrainerDashboard = ({ user }) => {
  ...
  const fetchTrainerData = async () => {
    try {
      ...
      // Fetch bookings - always fetch for trainers, regardless of profile existence
      // The bookings endpoint filters by trainer role automatically
      try {
        ...
        if (bookingsResponse.data && bookingsResponse.data.success && bookingsResponse.data.bookings) {
          console.log('Fetched bookings:', bookingsResponse.data.bookings.length);
      ...
      if (profileExists) {
        try {
          const sessionsResponse = await axios.get('/sessions/my-sessions');
                    ...
                    notifications.map(notification => (
                      <div key={notification._id} className={`notification-item ${!notification.isRead ? 'unread' : ''}`}>
                        <div className="notification-icon">
                          {notification.type === 'booking_new' ? '📅' : 'ℹ️'}
                        </div>
