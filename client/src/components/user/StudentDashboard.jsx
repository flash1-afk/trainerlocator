...
const StudentDashboard = ({ user }) => {
  ...
  const getHourlyRate = (services) => {
    if (services && services.length > 0) {
      // Return the price of the first service to match the default selected service in booking
      return services[0].price || 0;
    }
    return Math.floor(Math.random() * 2000) + 1000; // Random price between 1000-3000 for demo
  };
  ...
  const filteredTrainers = trainers.filter(trainer => {
    ...
    const matchesPrice = !priceRange ||
      (priceRange === 'low' && trainerRate < 2000) ||
      (priceRange === 'medium' && trainerRate >= 2000 && trainerRate < 5000) ||
      (priceRange === 'high' && trainerRate >= 5000);
  ...
  const handleBookingSubmit = async (e) => {
    ...
    try {
      ...
      const bookingPayload = {
        trainerId: typeof selectedTrainer.userId === 'object' ? (selectedTrainer.userId._id || selectedTrainer.userId.id) : selectedTrainer.userId,
        trainerProfileId: selectedTrainer._id,
          ...
          <div className="filters-grid">
            ...
            <div className="filter-group">
              ...
              >
                ...
                <option value="low">Under Rs. 2000/hr</option>
                <option value="medium">Rs. 2000 - 5000/hr</option>
                <option value="high">Rs. 5000+/hr</option>
              </select>
            </div>
                ...
                <div className="trainer-info">
                  <div className="trainer-header">
                    ...
                    <div className="trainer-price">
                      <div className="trainer-price-amount">Rs. {getHourlyRate(trainer.services)}</div>
          ...
          <div className="modal-content booking-modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="trainer-summary">
              ...
              <div className="trainer-modal-info">
                ...
                <p className="rate">Rs. {getHourlyRate(selectedTrainer.services)}/hour</p>
              </div>
            </div>
            ...
            <form onSubmit={handleBookingSubmit} className="booking-form">
                            ...
                            {selectedTrainer.services && selectedTrainer.services.length > 0 ? (
                              selectedTrainer.services.map((service, serviceIndex) => (
                                <option key={serviceIndex} value={service.name}>
                                  {service.name} - Rs. {service.price}/hr
                                </option>
                              ))
                            ) : (
                  ...
                  {bookingData.sessions.map((session, index) => {
                    ...
                    return (
                      <div key={index} className="price-item">
                        ...
                        <span>Rs. {(servicePrice * (session.duration / 60)).toFixed(2)}</span>
                      </div>
                    );
                  })}
                </div>
                <div className="total-price">
                  <strong>Total: Rs. {calculateTotalPrice().toFixed(2)}</strong>
                </div>
              </div>
              ...
              <div className="form-actions">
                ...
                <button
                  ...
                  disabled={bookingLoading}
                >
                  {bookingLoading ? 'Creating Booking...' : `Book Sessions - Rs. ${calculateTotalPrice().toFixed(2)}`}
                </button>
              </div>
            </form>
          </div>
